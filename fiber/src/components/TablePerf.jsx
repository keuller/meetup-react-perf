...
class TablePerf extends Component {
    ...
    componentDidMount() {
        this.bench = setInterval(() => {
            ...
            if (this.count > TIMES) {
                ...
                bench.delay(() => console.log('Average Load:', (this.totalLoad / TIMES)), 100)
            }
        }, 750)
    }
    ...
    render() {
        ...
        return (
            <div className="row">
                <button className="btn btn-secondary" disabled={this.state.contacts.length > 1} onClick={this.onLoad}>Load</button>
                <button className="btn btn-primary" disabled={this.state.contacts.length < 1} onClick={this.onClear}>Clear</button>
